test_that("Metropolis-Hastings samples standard normal target density", {
  ...
  expect_equal(length(res$accepted), 3000)
})
...
test_that("2D Ising model grid returns valid spin dimensions and boundary conditions", {
  grid <- ising_simulation(size = 8, beta = 0.44, n_steps = 1000)
  expect_equal(dim(grid), c(8, 8))
  expect_true(all(grid %in% c(-1, 1)))
})

test_that("CFTP perfect sampler coalesces within maximum lookback", {
  res <- cftp_perfect_sampler(max_lookback = 128)
  expect_true(is.list(res))
  expect_true(res$coalesced)
  expect_true(res$exact_sample %in% c(0, 1))
})
